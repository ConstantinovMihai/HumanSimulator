...
def read_data(data_signal, tc_signal='/home/mihai/Thesis/Data/Clean_CSV_data/updated_data/PRDPE/mdist.csv'):
    ...
    data = np.loadtxt(data_signal, delimiter=',')[:,0]
    tc = np.loadtxt(tc_signal, delimiter=',', dtype=int)[:,0]
...
if __name__ == '__main__':
    ...
    if theirs: 
        ...
        data = data[::10]
        # now cps has to be divided by 10 (and forced to int)
        cps = cps // 10

        data = data[:500]
        cps = cps[cps < 500]

        mean0 = np.mean(data[:100])
        varx = np.var(data[:100])
