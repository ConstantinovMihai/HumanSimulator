...
def simulate_closed_loop_dynamics(f_star, H1, H2, remnant,  ct: constants.SimulationConstants, print_matrices_dim=False):
    ...
    for t in range(len(f_star)-1):
        ...
        e = (f_star[t+1] - (u_star[t] + remnant[t+1] + 0)).reshape(-1, 1) # NOTE: the 0 is the disturbance signal (not implemented yet) 
        error_signal[t] = e 

    return u, u_star, error_signal
...
def run_test( plot_bode = False, plot_response = False, inject_remnant=True, test_remnant=False):
    ...
    H_remnant = {"num": np.array([ct.Kn_remnant]), "den": np.array([ct.Tn_remnant, 1]), "d0": np.array([0])} 

    H = {"num": np.array([ct.K_v, ct.K_p]), "den": np.array([1]), "d0": np.array([0])}
    
    # multiply the transfer functions
    H = sf.multiply_transfer_functions(H, ct.H_nms)
    H = sf.multiply_transfer_functions(H, ct.H_pade_v)

    H_ce = {"num": np.array([1.5]), "den": np.array([1, 0]), "d0": np.array([0])}

    sf.print_transfer_function(H)
    sf.print_transfer_function(H_ce)
    ...
    u, u_star, error_signal = simulate_closed_loop_dynamics(f_star, H, H_ce, remnant_realisation, ct)
    ...
    if test_remnant:
        open_loop_identification(remnant_realisation, white_noise, input_frequencies, H_remnant, ct, show_plot=True)

    open_loop_identification(u_star, u, input_frequencies, H_ce, ct, show_plot=True)
    open_loop_identification(u, error_signal, input_frequencies, H, ct, show_plot=True)
