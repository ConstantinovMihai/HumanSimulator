...
def get_fft_values(y_values : np.array, fs: float, N : int):
    ...
    Returns:
        frequency_values (numpy array): frequency values in Hz
...
if __name__ == "__main__":
    ...
    ft = sf.read_undistracted_data(f'Data/Clean_CSV_data/van_der_El_CSV_data/PRM/ft.csv')[1]
    
    frequency_values, fft_values = get_fft_values(ft, ct.fs, len(ft))
    
    plot_time_frequency_signal(np.arange(0, len(ft) * ct.dt, ct.dt), ft, frequency_values, fft_values, name="van der El ft signal")